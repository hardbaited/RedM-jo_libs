...
function jo.framework:getItemsFromInventory(invId)
  ...
  local invItems = MySQL.scalar.await("SELECT data FROM gm_inventory WHERE name = ?", { invId })
...
function jo.framework:standardizeSkinInternal(skin)
  ...
  standard.model = table.extract(skin, "gender") == 2 and "mp_female" or "mp_male"
  ...
  standard.overlays = {}
  standard.overlays.ageing = skin.ageing_type and {
    id = getStandardValuefromFramework("ov_ageing", skin.ageing_type) or 0,
    opacity = self:convertToPercent(skin.ageing_opa)
  }

  standard.overlays.beard = skin.beard and {
    id = skin.beard,
  ...
  standard.overlays.eyebrow = skin.eyebrows_type and (function()
    local standardEyebrow = getStandardValuefromFramework("ov_eyebrows", skin.eyebrows_type)
    return {
      id = standardEyebrow?.id or 0,
      sexe = standardEyebrow?.sexe or "m",
      palette = getStandardValuefromFramework("ov_palette", skin.eyebrows_id)?.palette or skin.eyebrows_pal_id,
  ...
  standard.overlays.eyeliner = skin.eyeliners_type and {
    id = 0,
    sheetGrid = skin.eyeliners_type,
  ...
  standard.overlays.eyeshadow = skin.shadows_type and {
    id = 0,
    sheetGrid = skin.shadows_type,
...
function jo.framework:revertSkinInternal(standard)
  ...
  reverted.gender = standard.model == "mp_female" and "female" or "male"
  ...
  reverted.hair = type(standard.hair) == "table" and standard.hair.hash or standard.hair
  if standard.model == "mp_male" and type(standard.beards_complete) == "table" then
    reverted.beard = standard.beards_complete.hash
  end
...
function jo.framework:getUserClothesInternal(source)
  ...
  local clothes = MySQL.scalar.await("SELECT clothes FROM characters WHERE characterid=?", { user.identifier })
...
function jo.framework:updateUserClothesInternal(source, clothes)
  ...
  MySQL.scalar("SELECT clothes FROM characters WHERE characterid=? ", { identifiers.identifier }, function(oldClothes)
    local decoded = UnJson(oldClothes)
    table.merge(decoded, clothes)
    -- MySQL.update("UPDATE characters SET clothes=? WHERE characterid=?", { json.encode(decoded), identifiers.identifier })
    exports.gm_core:SetPlayerState(source, "clothes", decoded)
  end)
end
...
function jo.framework:getUserSkinInternal(source)
  ...
  local skin = MySQL.scalar.await("SELECT skin FROM characters WHERE characterid=?", { identifiers.identifier })
...
function jo.framework:updateUserSkinInternal(source, skin, overwrite)
  local identifiers = self:getUserIdentifiers(source)
  if overwrite then
    -- exports.gm_core:SetPlayerState(source, "skin", skin)
  else
    MySQL.scalar("SELECT skin FROM characters WHERE characterid=?", { identifiers.identifier }, function(oldSkin)
      local decoded = UnJson(oldSkin)
      table.merge(decoded, skin)
      -- exports.gm_core:SetPlayerState(source, "skin", decoded)
    end)
  end
end
...
function jo.framework:onCharacterSelected(cb)
  RegisterNetEvent("gm_core:client:playerLoaded", function()
    local source = source
    cb(source)
  end)
end